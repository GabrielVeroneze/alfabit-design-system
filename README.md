<h1 align="center">
  <img alt="Alfabit Design System" title="#alfabit-ui" src="public/logo-alfabit-branco.png" height="150" />
</h1>
...
<p align="center">
 <img alt="Repository size" src="https://img.shields.io/github/repo-size/GabrielVeroneze/alfabit-design-system?color=4e5acf">

  <a aria-label="Last Commit" href="https://github.com/GabrielVeroneze/alfabit-design-system/commits/main">
  <img alt="Last commit on GitHub" src="https://img.shields.io/github/last-commit/GabrielVeroneze/alfabit-design-system?color=4e5acf">
...
A biblioteca Alfabit-ui foi criada para funcionar em aplicações Next.js+TailwindCSS. Portanto, para usá-la lembre-se de criar suas aplicações com essa estrutura. Para que você consiga usar todos os componentes da biblioteca sem problemas é necessário uma configuração adicional no seu projeto. Vamos fazer isso passo a passo:
...
1.  No mesmo arquivo que você importa as camadas do tailwind (geralmente o arquivo `globals.css` ou `styles.css` ) importe o arquivo CSS da biblioteca Alfabit também
    ...
    ```tsx
    import './globals.css'
    import 'alfabit-ui/styles/globals.css'
    ```
...
2.  Dentro do arquivo de configuração do Tailwind, o `tailwind.config.js`, adicione as configurações da biblioteca Alfabit como `preset`:
    ...
    ```tsx
    const config = {
        presets: [require('alfabit-ui/tailwind.config')],
...
3.  Adicione no mesmo arquivo de configuração a referência dos arquivos da biblioteca Alfabit para serem cobertas pelo Tailwind e processadas no PostCSS. Para isso, basta adicionar o caminho dos componentes da biblioteca Alfabit dentro da propriedade `content`, no mesmo arquivo de configuração do tailwind do passo anterior:
    ...
    const config = {
        content: [
            ...
            './node_modules/alfabit-ui/components/**/*.{js,ts,jsx,tsx,mdx}',
        ],
        // ... demais configurações ...
    }
    ```
...
```tsx
'use client'
import { Typography } from 'alfabit-ui/components'
...
> Confira também a documentação da biblioteca para mais detalhes de uso [Documentação](https://alfabit-design-system-app.vercel.app/?path=/docs/design-tokens-colors--docs).
...
Para verificar a documentação click [aqui](https://alfabit-design-system-app.vercel.app/?path=/docs/design-tokens-colors--docs)
